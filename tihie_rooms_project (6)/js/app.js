...
function setTheme(idx){
  ...
  if (btn) btn.innerHTML = '\u{1F3A8} ' + themeNames[currentThemeIdx] + ' [' + (currentThemeIdx + 1) + '/25]';var pp=getProfile();if(pp){if(!pp._themes)pp._themes={};pp._themes[t]=true;saveProfile(pp);if(Object.keys(pp._themes).length>=25)awardAchievement('stylist');}
}
...
// ── PROFILE BUTTON ──────────────────────────────
document.getElementById('profileBtn').addEventListener('click',function(){openProfileModal();});
...
function switchSection(section){
  ...
  var labels = {home:'TERMINAL',levels:'LEVELS',entities:'ENTITIES',items:'OBJECTS',foods:'SUSTENANCE',bookmarks:'BOOKMARKS',timeline:'TIMELINE',changelog:'CHANGELOG',researchers:'RESEARCHERS'};
  breadcrumbs.innerHTML = section === 'home' ? 'TERMINAL' : '<span onclick="window._sw(\'home\')">TERMINAL</span> \u203A ' + labels[section];
  var needsLoad = ['home','bookmarks','timeline','changelog','researchers'].indexOf(currentSection)!==-1
  if (needsLoad) {
    var load = showLoading(contentArea);
...
// ── KONAMI CODE ─────────────────────────────────
var konamiBuf = '';awardAchievement('konami_master');
