...
function loadSectionData(section){
  return fetchFileIndex(section).then(function(files){
    if(!files||!files.length){
      console.warn('TIHIE: No files found for section '+section);
      data[section]=[];
      return [];
    }
    return Promise.all(files.map(function(f){
      return fetch(f.download_url,{cache:'no-store'})
        ...
        .then(function(obj){if(obj){obj._sha=f.sha;obj._folder=section;return obj;}return null;})
        .catch(function(e){console.warn('TIHIE: Failed to load '+f.name+':',e.message||e);return null;});
    }));
  }).then(function(results){
    var arr=(results||[]).filter(Boolean);
    console.log('TIHIE: Loaded '+arr.length+' records for section '+section);
    ...
    console.error('TIHIE: Section '+section+' failed to load:',e.message||e);
    data[section]=[];
    return [];
  });
}
function loadAllData(){
  ...
  return Promise.all(sections.map(function(s){
    return loadSectionData(s).catch(function(e){
      console.warn('TIHIE: loadAllData — section '+s+' error:',e.message||e);
      data[s]=[];
      return [];
    });
  }));
